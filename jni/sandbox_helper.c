...
pid_t ptrace_app_process(pid_t process_pid, int flag)
{
	...
	while (pid > 0) {
		if (IS_FORK_EVENT(status) || IS_CLONE_EVENT(status)) {
			...
			switch (syscall_no) {
			case __NR_setuid32:
				if (syscall_setuid_gid_handler(pid, process_pid, 0,
						(long *) &uid) == 0) {
					printf("%d exit\n", getpid());
					return -1;
				} else {
			...
			case __NR_setgid32:
				if (syscall_setuid_gid_handler(pid, process_pid, 1,
						(long *) &gid) == 0) {
					printf("%d exit\n", getpid());
					return -1;
				} else {
...
void syscall_common_handler(pid_t pid, char* syscall, int flag, uid_t uid, gid_t gid)
{
	...
	if ((flag & SANDBOX_FLAG) && FILE_SANDBOX_ENABLED && (nth_dir =
			check_prefix_dir(path, SANDBOX_PATH_INTERNAL)) > 0) {
		...
		if (!check_prefix(sub_dir, SANDBOX_PATH_INTERNAL_EXCLUDE)) {
			...
			printf("pid %d %s: %s ==> new path: %s", pid, syscall, path,
					new_path);
...
void syscall_open_handler(pid_t pid, int *binder_fd, int flag, uid_t uid,
		gid_t gid)
{
	tracee_ptr_t arg0 = (tracee_ptr_t) ptrace_get_syscall_arg(pid, 0);
	//arg1 is oflag
...
static volatile bool_t *s_sandbox_enabled;

void init_sandbox_state()
{
	s_sandbox_enabled = mmap(NULL, sizeof *s_sandbox_enabled,
			PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	*s_sandbox_enabled = false;
}

bool_t is_sandbox_enabled()
{
	printf("dddddddddddddddddddddddddd:%d,%d\n", *s_sandbox_enabled, getpid());
	return *s_sandbox_enabled;
}

void set_sandbox_enabled(bool_t enabled)
{
	*s_sandbox_enabled = enabled;
	printf("ssssssssssssssssssssssssss:%d,%d\n", *s_sandbox_enabled, getpid());
}
